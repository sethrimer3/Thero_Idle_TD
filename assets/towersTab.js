...
function getUniversalVariableMetadata(variableOrKey) {
  ...
  return UNIVERSAL_VARIABLE_LIBRARY.get(lookupKey) || null;
}
...
function createDiscoveredVariableRecord(towerId, variable, variableId) {
  ...
  return {
    ...
    towerOrder: towerTabState.towerOrderIndex.get(towerId) ?? Number.MAX_SAFE_INTEGER,
...
function notifyDiscoveredVariableListeners() {
  if (!towerTabState.discoveredVariableListeners.size) {
    return;
  }
...
function discoverTowerVariables(towerId, { notify = true } = {}) {
  ...
  blueprint.variables.forEach((variable) => {
    ...
    if (!variableId || towerTabState.discoveredVariables.has(variableId)) {
      return;
    }
    ...
    towerTabState.discoveredVariables.set(variableId, record);
    changed = true;
  });
  if (changed && notify) {
...
export function getDiscoveredVariables() {
  const entries = Array.from(towerTabState.discoveredVariables.values());
  entries.sort((a, b) => {
...
export function initializeDiscoveredVariablesFromUnlocks(unlockCollection) {
  towerTabState.discoveredVariables.clear();
  const unlocked = normalizeTowerIdCollection(unlockCollection);
  if (!unlocked.size && towerTabState.unlockState?.unlocked instanceof Set) {
    towerTabState.unlockState.unlocked.forEach((towerId) => {
      if (typeof towerId === 'string' && towerId.trim()) {
        unlocked.add(towerId.trim());
      }
    });
  }
  const orderedDefinitions = towerTabState.towerDefinitions.length
  ...
  if (!orderedDefinitions.length && unlocked.size) {
