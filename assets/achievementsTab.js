...
let jsonAchievements = []; // Store loaded JSON achievements
...
export async function loadAchievementsFromJSON() {
  try {
    ...
    if (!response.ok) {
      console.warn('Could not load achievements.json, using level-only achievements');
      return [];
    }
    const data = await response.json();
    jsonAchievements = data.sort((a, b) => (a.order || 0) - (b.order || 0));
...
// Creates condition checker functions based on conditionType
function createConditionChecker(achievement) {
  ...
  switch (conditionType) {
    case 'level-complete':
      return () => {
        if (typeof ctx.isLevelCompleted === 'function') {
          return ctx.isLevelCompleted(conditionValue);
        }
        return false;
      };
    
    case 'towers-simultaneous':
      return () => {
        const { gameStats } = ctx;
        return gameStats && gameStats.maxTowersSimultaneous >= conditionValue;
      };
    
    case 'powder-multiplier':
      return () => {
        const { gameStats } = ctx;
        return gameStats && gameStats.maxPowderMultiplier >= conditionValue;
      };
    
    case 'enemies-defeated':
      return () => {
        const { gameStats } = ctx;
        return gameStats && gameStats.enemiesDefeated >= conditionValue;
      };
    
    case 'auto-anchor-complete':
      return () => {
        const { gameStats } = ctx;
        return gameStats && gameStats.autoAnchorPlacements >= (gameStats.autoAnchorTotal || 0) && gameStats.autoAnchorTotal > 0;
      };
    
    case 'sigils-illuminated':
      return () => {
        const { gameStats } = ctx;
        return gameStats && gameStats.sigilsIlluminated >= conditionValue;
      };
    
    case 'idle-victories':
      return () => {
        const { gameStats } = ctx;
        return gameStats && gameStats.idleVictories >= conditionValue;
      };
    
    case 'idle-time-minutes':
      return () => {
        const { gameStats } = ctx;
        return gameStats && (gameStats.idleTimeSeconds || 0) >= (conditionValue * 60);
      };
...
// Creates progress text for JSON achievements
function createProgressChecker(achievement) {
  const { conditionType, conditionValue, title } = achievement;
  ...
  return () => {
    ...
    if (!gameStats) {
      return `Locked — ${title} to unlock.`;
    }
    
    switch (conditionType) {
      case 'level-complete':
        return `Locked — Complete ${conditionValue} to unlock.`;
      
      case 'towers-simultaneous': {
        const current = gameStats.maxTowersSimultaneous || 0;
        return `Locked — ${current}/${conditionValue} towers simultaneously.`;
      }
      
      case 'powder-multiplier': {
        const current = gameStats.maxPowderMultiplier || 1;
        return `Locked — ${current.toFixed(2)}×/${conditionValue}× multiplier.`;
      }
      
      case 'enemies-defeated': {
        const current = gameStats.enemiesDefeated || 0;
        return `Locked — ${current}/${conditionValue} enemies defeated.`;
      }
      
      case 'auto-anchor-complete':
        return `Locked — Use all auto-lattice anchors in a stage.`;
      
      case 'sigils-illuminated': {
        const current = gameStats.sigilsIlluminated || 0;
        return `Locked — ${current}/${conditionValue} sigils illuminated.`;
      }
      
      case 'idle-victories': {
        const current = gameStats.idleVictories || 0;
        return `Locked — ${current}/${conditionValue} idle victories.`;
      }
      
      case 'idle-time-minutes': {
        const currentMinutes = Math.floor((gameStats.idleTimeSeconds || 0) / 60);
        return `Locked — ${currentMinutes}/${conditionValue} minutes of idle time.`;
      }
...
// Converts JSON achievement to internal definition format
function createJSONAchievementDefinition(jsonAch) {
  return {
    id: jsonAch.id,
    title: jsonAch.title,
    subtitle: jsonAch.title,
    icon: jsonAch.icon,
    rewardFlux: jsonAch.reward || ACHIEVEMENT_REWARD_FLUX,
    description: `${jsonAch.description} Unlocking adds +${jsonAch.reward || ACHIEVEMENT_REWARD_FLUX} Motes/min to idle reserves.`,
    condition: createConditionChecker(jsonAch),
    progress: createProgressChecker(jsonAch),
  };
}
...
// Recomputes the full achievements list using JSON achievements and level order.
export function generateLevelAchievements() {
  ...
  // Add JSON-defined achievements first
  jsonAchievements.forEach((jsonAch) => {
    definitions.push(createJSONAchievementDefinition(jsonAch));
  });

  // Then add level-based achievements
